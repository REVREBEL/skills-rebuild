...
Ou crie um arquivo `.env` em `{SKILL_DIR}/`:
```
...
```bash
python {SKILL_DIR}/scripts/generate.py --prompt "mulher jovem tomando cafe em cafeteria" --mode influencer --format square
```
...
```bash
python {SKILL_DIR}/scripts\prompt_engine.py --prompt "prompt do usuario" --mode influencer
```
...
```bash
python {SKILL_DIR}/scripts\templates.py --list
```
