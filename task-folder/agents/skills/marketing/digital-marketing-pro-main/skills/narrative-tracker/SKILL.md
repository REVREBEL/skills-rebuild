...
1. **Load brand context**: Read `~/.agents-marketing/brands/_active-brand.json` for the active slug, then load `~/.agents-marketing/brands/{slug}/profile.json`. Extract brand positioning, key messages, differentiators, value propositions, target audience, and competitive claims — these form the reference narrative against which AI responses are evaluated. Also check for guidelines at `~/.agents-marketing/brands/{slug}/guidelines/_manifest.json` — if present, load messaging dos/don'ts and positioning guardrails. If no brand exists, ask: "Set up a brand first (/digital-marketing-pro:brand-setup)?" — or proceed with user-provided positioning statements.
