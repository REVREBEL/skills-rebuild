...
13. **Log the update**: Record the complete update — timestamp, deal ID, before state, after state, user who initiated, velocity impact, follow-up tasks created, win/loss data, and notifications sent — to `~/.agents-marketing/brands/{slug}/logs/pipeline-update-log.json`.
