...
13. **Log results and notify**: Record the complete sync execution — timestamp, record counts, field mapping, errors, relationship links created, and duration — to `~/.agents-marketing/brands/{slug}/logs/crm-sync-log.json`. If notification preferences were set, alert relevant sales reps about new records in their pipeline.
