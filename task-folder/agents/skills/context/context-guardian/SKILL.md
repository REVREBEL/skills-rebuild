...
```
{USER_HOME}\skills\context-guardian\
...
```bash
python {USER_HOME}\skills\context-guardian\scripts\context_snapshot.py save
```

Gera `{USER_HOME}\skills\context-guardian\data\snapshot-YYYYMMDD-HHMMSS.md` com
...
- Snapshot: {USER_HOME}\skills\context-guardian\data\snapshot-[timestamp].md
...
5. Se ainda houver lacunas, buscar por termo:
   ```bash
   python {USER_HOME}\skills\context-agent\scripts\context_manager.py search "termo"
   ```
