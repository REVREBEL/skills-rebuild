...
|-------|--------|-----|----------------|
| 1 | `CLAUDE_CODE` env var set (any value) | the agent desktop | Print `claude-code://browser?url=http://localhost:<port>` as a clickable hint; the agent's desktop app intercepts `claude-code://` URLs. |
...
| 3 | `TERM_PROGRAM=vscode` AND no Cursor/the agent signal | Plain VS Code | Print the URL with a hint: `Open in VS Code: Ctrl+Shift+P → "Simple Browser: Show" → paste URL`. |
