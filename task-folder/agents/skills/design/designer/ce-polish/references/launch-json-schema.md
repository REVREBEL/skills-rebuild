...
Polish reads `.agents/launch.json` at the repo root to resolve the dev-server start command. The schema is a subset of VS Code's `launch.json` format — chosen because the agent, Cursor, and VS Code all understand it and because users often already have one for editor integration.
...
`.agents/launch.json` is not yet a fully unified standard across the agent, Cursor, VS Code, and Codex. Polish leads with `.agents/launch.json` because:
