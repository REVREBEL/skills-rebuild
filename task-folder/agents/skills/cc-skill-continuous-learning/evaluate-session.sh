...
#
# Hook config (in ~/.agents/settings.json):
# {
#   "hooks": {
#     "Stop": [{
#       "matcher": "*",
...
LEARNED_SKILLS_PATH="${HOME}/.agents/skills/learned"
MIN_SESSION_LENGTH=10

# Load config if exists
if [ -f "$CONFIG_FILE" ]; then
  MIN_SESSION_LENGTH=$(jq -r '.min_session_length // 10' "$CONFIG_FILE")
  LEARNED_SKILLS_PATH=$(jq -r '.learned_skills_path // "~/.agents/skills/learned/"' "$CONFIG_FILE" | sed "s|~|$HOME|")
fi
