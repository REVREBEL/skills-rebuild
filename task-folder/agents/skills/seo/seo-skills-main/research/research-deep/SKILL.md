...
- **`{item_name_slug}` collisions silently overwrite.** Two items whose names slugify to the same string (e.g. `the active model` and `GPT 4`) will clobber each other. If you suspect collision, add a discriminator in the item's `category` field and slug them as `{category_slug}_{name_slug}` instead.
