...
main() {
    ...
    # Remove skill
    rm -rf "${HOME}/.agents/skills/seo-dataforseo"

    # Remove agent
    rm -f "${HOME}/.agents/agents/seo-dataforseo.md"

    # Remove field config
    rm -f "${HOME}/.agents/skills/seo/dataforseo-field-config.json"
...
" 2>/dev/null || echo "  ⚠  Could not auto-remove MCP config. Remove 'dataforseo' from ~/.agents/settings.json manually."
    fi
