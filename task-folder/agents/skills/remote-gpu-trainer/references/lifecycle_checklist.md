...
- [ ] Use `run_in_background` (no duration cap, notifies on exit; a the agent primitive — other hosts map per `monitoring_patterns.md` §7) for long waits; never foreground-poll. NEVER an unquoted `|` inside a poll-regex — it reads stdin and hangs forever.
