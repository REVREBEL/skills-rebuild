...
| the agent | `~/.agents/skills/` | **Symlink → `~/.agents/skills/`** — writing to `.agents/skills` automatically covers Claude |
...
2. **Verify the `.agents` symlink is intact** (one-time check):
   ```bash
   ls -la ~/.agents/skills
   # Expect: ~/.agents/skills -> ~/.agents/skills
   ```
   If it's a real directory instead of a symlink, the user has diverged copies — ask before touching.
3. **Copy to `.hermes` only** (`.agents` and `.pi` are symlinks — already covered):
   ```bash
   SKILL=<skill-name>
...
4. **Verify all 4 locations** show identical byte counts:
   ```bash
   for p in ~/.agents/skills/$SKILL ~/.agents/skills/$SKILL ~/.pi/agent/skills/$SKILL ~/.hermes/skills/$SKILL; do
     echo "$p: $(wc -c < $p/SKILL.md) bytes"
   done
   ```
   All four numbers must match. If `.agents` or `.pi` shows a different byte count, that symlink is broken — investigate before proceeding.

## Updating an Existing Distributed Skill

Same flow — re-copy from `~/.agents/skills/` to `.hermes/skills/`. The `.agents` and `.pi` symlinks update automatically. `cp -r` overwrites by default; use `rsync -a --delete` if the skill folder has nested files that may have been removed:
...
- **`~/.agents/skills` is a symlink, not a folder.** `cp -r ~/.agents/skills/foo ~/.agents/skills/` will error with "are identical". Skip the explicit Claude copy.
...
- Skill is project-specific → put it in `./.agents/skills/`, `./.pi/agent/skills/`, etc. inside the repo, not globally.
