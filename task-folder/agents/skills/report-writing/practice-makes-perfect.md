...
The skill files live in `projects/pentest/.claude/skills/` — the same directory where the agent runs during benchmarks. To update them, we used the agent itself with the `/skill-update` command, which enforces structural constraints:
