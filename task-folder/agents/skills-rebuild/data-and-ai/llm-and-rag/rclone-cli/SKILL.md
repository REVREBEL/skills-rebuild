...
```bash
rclone sync --dry-run -P ~/documents remote:backup/documents
# Review dry-run output carefully, then:
rclone sync -P ~/documents remote:backup/documents
```
...
### Restore from Cloud
```bash
rclone copy --dry-run remote:backup/documents ~/restored
rclone copy -P remote:backup/documents ~/restored
```
