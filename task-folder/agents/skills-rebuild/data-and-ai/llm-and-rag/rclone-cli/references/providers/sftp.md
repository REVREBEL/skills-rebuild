...
(`i.e ~`). However, `rclone lsd remote:/` would list the root
...
Sync `~/directory` to the remote directory, deleting any
excess files in the directory.

```console
rclone sync --interactive ~/directory remote:directory
```
...
as with PowerShell. Rclone has subcommands `hashsum`,
`md5sum` and `sha1sum`
...
E.g. if shared folders can be found in directories representing volumes:
    ...
    rclone sync ~/directory remote:/directory --sftp-path-override /volume2/directory

E.g. if home directory can be found in a shared folder called "home":

    rclone sync ~/directory remote:~ --sftp-path-override /volume1/homes/USER/directory
...
E.g. the first example above could be rewritten as:
	...
	rclone sync ~/directory remote:/directory --sftp-path-override @/volume2
...
E.g. the second example above should be rewritten as:
	...
	rclone sync ~/directory remote:/homes/USER/directory --sftp-path-override @/volume1
...
the value. The server run by `rclone serve sftp`
