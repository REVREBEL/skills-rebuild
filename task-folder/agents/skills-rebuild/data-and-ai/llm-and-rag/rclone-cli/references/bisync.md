...
- Bisync will create its working directory   at `~/.cache/rclone/bisync` on
  Linux, `~/Library/Caches/rclone/bisync` on Mac,   or
  `~\AppData\Local\rclone\bisync` on Windows.   Make sure that this
...
should not be accessible. See also the `--check-filename` flag.
...
up considerably, especially on backends (such as `local`) where hashes
...
advantage over methods like ``cryptcheck`` is
...
``cryptcheck`` command, ``rclone check
--download`` option,
``md5sum`` command
...
changed with the ``--suffix-keep-extension`` flag
...
``--sufix`` flag. This is intentional, as users may wish
...
just their listing snapshots), consider using ``check``
(or ``cryptcheck``, if at least one path is a
...
As of `v1.66`, ``--backup-dir`` is supported in bisync.
...
moves it from `~/some/local/path/Bisync` to
`~/some/local/path/BackupDir`.
...
``check``.) If bisync concludes that the files are
...
NOT use ``--inplace``, otherwise you risk leaving
...
which works in much the same way as in ``sync`` and
``copy``. When used, empty directories created or
...
- ``--local-unicode-normalization`` and
``--local-case-sensitive`` (caution: these are
...
Working directory is created at `~\AppData\Local\rclone\bisync`.
...
- Directory trees containing application temporary files and data
  such as the Windows `~\AppData\` tree.
...
set to `~`. The strategy is to select specific directories
...
- Windows creates cache files starting with both upper and
  lowercase `NTUSER` at `~`. These files may be dynamic,
...
`~\My Documents\My Music` pointing to `~\Music`.
...
```text
0 * * * * /path/to/rclone bisync /path/to/local/dropbox Dropbox: --check-access --filters-file ~/filters.txt >> /path/to/logs/dropbox-run.log 2>&1
```
...
For the sensitive data, configure an rclone `crypt remote` to point to
...
- The Dropbox client runs keeping the local tree `~\Dropbox`
  always in sync with Dropbox. I could have used `rclone bisync` instead.
- A separate directory tree at `~\Documents\DropLocal`
...
- To sync my local unencrypted files with the encrypted
  ...
  `rclone bisync ~\Documents\DropLocal Dropcrypt:`.
...
remote = ~\Dropbox\crypt   # on the Windows notebook
