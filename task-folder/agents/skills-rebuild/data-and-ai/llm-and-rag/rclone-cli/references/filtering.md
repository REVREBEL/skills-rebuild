...
`rclone copy --files-from files-from.txt ~/pics remote:pics`
copies the following, if they exist, and only those files.

```text
~/pics/file1.jpg        → remote:pics/file1.jpg
~/pics/subdir/file2.jpg → remote:pics/subdir/file2.jpg
```
...
```text
~/42       → remote:backup/user1/important
~/dir/ford → remote:backup/user1/dir/file
~/prefect  → remote:backup/user2/stuff
```
...
```text
~/42       → remote:backup~/42
~/dir/ford → remote:backup~/dir/ford
~/prefect  → remote:backup~/prefect
```
...
with `;` or `#` are read without any processing. `rclone lsf`
...
name filters, except they match `metadata` on the
object.
