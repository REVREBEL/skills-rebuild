...
|---|---|---|---|---|
| Local / root NVMe | `/`, `~` | yes (data persists; **but cold reboot wipes RAM** — LAM9) | **NO** (erased, unrecoverable) | instance root volume |
...
- **Disk pressure on the ephemeral root:** `df -h /` and `df -h /lambda/nfs/<name>`; remember `~`
...
```
DATA_DIR=       ~ (ephemeral NVMe — lost on terminate)
...
- Default checkpoint dir → the NFS mount, not `~` — the latter is erased on terminate.
