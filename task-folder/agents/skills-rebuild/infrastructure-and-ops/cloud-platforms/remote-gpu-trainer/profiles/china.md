...
shared_fs: per-platform      # /mnt | /hy-netdisk(+/hy-nas) | work+/cloud | ~/netdisk/data — region/machine-scoped, see §2
...
local_nvme: per-platform     # Gpushare /hy-tmp, LanRui ~/datadisk, Featurize local scratch
---
...
| Featurize | wiped on return | **`work` = `~`** (persists) + **`/cloud`** sync drive | local (non-persistent) | **30 GB** free cloud |
| LanRui | system disk lost on stop | **数据盘 = `~/datadisk`** + shared **`~/netdisk/data`** | 数据盘 (block-storage, ≈ sys-disk speed) | 网盘 10 GB free |
...
- **LanRui** — `~/netdisk/data` is a **per-availability-zone shared folder auto-mounted into *every*
...
| Featurize | **实例归还** (return) | only `work` (`~`) + `/cloud` persist | everything else **wiped immediately on return** (FZ1) |
...
(`~/datadisk`, block storage, bought in 200 G / 500 G specs) bills hourly from *creation* until
...
"what survives" rule of the four.) Symptom: results outside `~` or `/cloud` gone after 归还.
...
(`~/datadisk`) bills hourly from creation until *destroyed*, independent of workspace run-state
...
a parallel ablation overwrites another run's outputs. → Root cause: `~/netdisk/data` is auto-mounted
...
|---|---|---|---|---|
| `DURABLE_DIR=` (durable) | `/mnt` | `/hy-netdisk` | `~` (+`/cloud`) | `~/datadisk` (or `~/netdisk/data`) |
| `DATA_DIR=` (fast/ephemeral) | `/root` | `/hy-tmp` (24 h post-stop wipe) | local tmp | `~/datadisk` scratch |
| `SCRATCH=` (local, prune) | `/root` | `/hy-tmp` | local tmp | 数据盘 scratch |
| `HF_HOME=` | `/mnt/.cache/hf` | `/hy-netdisk/.cache/hf` | `/cloud/.cache/hf` | `~/datadisk/.cache/hf` |
...
| Shared / region FS? | `/mnt` netdisk (**region-scoped**, expandable) | `/hy-netdisk` (only on *marked* machines) + `/hy-nas` | `work`+`/cloud` (cloud sync; not cross-region, med-conf) | `~/netdisk/data` (shared into *every same-zone* workspace) |
