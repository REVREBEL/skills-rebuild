...
- **Start here**: `Architecture`, `Development`, `ECL`
...
|---------|----------|-------------|
| 3.1 | `System Architecture` | Layer hierarchy, data flow, critical files |
| 3.2 | `Relevant Design Doc` | [Project-specific component purpose] |
| 3.3 | `API / Interface Reference` | Public routes, commands, or module interfaces |
| 3.4 | `Configuration` | Runtime env vars and service dependencies |

## 4 Context Loading

1. Read this file.
2. Read `ECL` for change lifecycle and context rules.
...
5. If no active change exists and no pending evolution exists, read `Status` for recent handoff context.
...
See `Development Setup` for complete reference.
...
> See `3.2 Error Codes` for the complete error taxonomy.
...
- `Architecture Overview` — Section 3.1
- `Related Component` — How it interacts with this component
```
