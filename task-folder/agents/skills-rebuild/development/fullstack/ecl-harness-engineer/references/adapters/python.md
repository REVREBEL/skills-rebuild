...
adapter:
  ...
  route_detection:
    ...
    patterns:
      # FastAPI
      - type: route
        regex: '@(?:app|router)\.(get|post|put|delete|patch)\s*\(\s*`"\x27`["\x27]'
        groups: [method, path]
        frameworks: ["fastapi", "starlette", "litestar"]
      # Flask
      - type: route
        regex: '@(?:app|bp|blueprint)\.(route)\s*\(\s*`"\x27``"\x27`\])?'
        groups: [_, path, method]
        frameworks: ["flask"]
      # Django URLs
      - type: route
        regex: 'path\s*\(\s*`"\x27`["\x27]'
        groups: [path]
        frameworks: ["django"]
      # Click
      - type: command
  ...
  dependency_detection:
    ...
    services:
      ...
      - pattern: "elasticsearch|elastic-transport"
        ...
        default_port: 9200
    env_var_patterns:
      - pattern: 'os\.environ\.get\(\s*`"\x27`["\x27]'
      - pattern: 'os\.environ\`["\x27`["\x27]\]'
      - pattern: 'os\.getenv\(\s*`"\x27`["\x27]'
