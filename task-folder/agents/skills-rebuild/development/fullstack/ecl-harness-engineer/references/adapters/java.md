...
adapter:
  ...
  route_detection:
    ...
    patterns:
      ...
      - type: route
        regex: '@(GetMapping|PostMapping|PutMapping|DeleteMapping|PatchMapping)\s*\(\s*(?:value\s*=\s*)?`"\x27`["\x27]'
        groups: [method, path]
        frameworks: ["spring"]
      # Spring RequestMapping
      - type: route
        regex: '@RequestMapping\s*\(.*(?:value|path)\s*=\s*`"\x27`["\x27].*method\s*=\s*RequestMethod\.(\w+)'
        groups: [path, method]
        frameworks: ["spring"]
      # Ktor (Kotlin)
      - type: route
        regex: '(get|post|put|delete|patch)\s*\(\s*`"\x27`["\x27]'
        groups: [method, path]
        frameworks: ["ktor"]
      # Javalin
      - type: route
        regex: 'app\.(get|post|put|delete|patch)\s*\(\s*`"\x27`["\x27]'
        groups: [method, path]
        frameworks: ["javalin"]
  ...
  dependency_detection:
    ...
    services:
      ...
      - pattern: "elasticsearch-rest-client|spring-data-elasticsearch"
        ...
        default_port: 9200
    env_var_patterns:
      - pattern: 'System\.getenv\(\s*`"\x27`["\x27]\)'
