...
adapter:
  ...
  route_detection:
    ...
    patterns:
      # Express / Koa style
      - type: route
        regex: '(app|router)\.(get|post|put|delete|patch|all)\s*\(\s*`"\x27`["\x27]'
        groups: [_, method, path]
        frameworks: ["express", "koa", "hono"]
      # Fastify
      - type: route
        regex: '(fastify|server|app)\.(get|post|put|delete|patch)\s*\(\s*`"\x27`["\x27]'
        groups: [_, method, path]
        frameworks: ["fastify"]
      # NestJS decorators
      - type: route
        regex: '@(Get|Post|Put|Delete|Patch)\s*\(\s*`"\x27`["\x27]?\s*\)'
        groups: [method, path]
        frameworks: ["nestjs"]
      ...
      # Commander CLI
      - type: command
        regex: '\.command\s*\(\s*`"\x27`["\x27]'
        groups: [command_name]
        frameworks: ["commander"]
      # Yargs
      - type: command
        regex: '\.command\s*\(\s*`"\x27`'
        groups: [command_name]
        frameworks: ["yargs"]
