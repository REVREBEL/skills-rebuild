...
- ``validate-profile`` — prerequisite BLOCKER check
- ``campaign-audit`` — produces the current-state baseline this launch differentiates from
- [`campaign-plan`](../campaign-plan/SKILL.md) — produces the plan this skill consumes
- ``launch-ad-campaign`` — the paid-ads-only subset that this skill delegates to for Step 5
- ``performance-check`` — day-1 check-in skill mentioned in the success message
- ``performance-monitor`` — arms the day-1 watchdog
- ``crm-sync`` — creates the CRM Campaign object
- ``c2pa-metadata`` — signs AI assets for EU launches (mandatory per Step 0)
