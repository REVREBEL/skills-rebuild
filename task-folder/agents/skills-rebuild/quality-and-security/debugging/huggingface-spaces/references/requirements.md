...
If a research repo ships a Python package directory (`models/`, `pipeline/`, …), just upload the directory with the rest of the Space — the whole repo root is importable as `~r/app`. **Do not** try to reference local paths from `requirements.txt`.
