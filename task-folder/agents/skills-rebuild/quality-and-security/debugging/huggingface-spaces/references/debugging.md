...
It's a normal container at `~r/app/`. You can edit files, `pip install`, run repros, call `@spaces.GPU`-decorated functions interactively (they get a real GPU window).
...
**Edits in `~r/app/` don't survive** a restart, sleep, or dev-mode-disable. Only commits persist.
