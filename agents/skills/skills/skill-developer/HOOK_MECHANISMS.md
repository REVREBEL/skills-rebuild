...
User submits prompt
    ↓
.agents/settings.json registers hook
    ↓
...
Outputs formatted message to stdout
    ↓
stdout becomes context for Agent (injected before prompt)
    ↓
Agent sees: [skill suggestion] + user's prompt
```

### Key Points

- **Exit code**: Always 0 (allow)
- **stdout**: → Agent's context (injected as system message)
- **Timing**: Runs BEFORE Agent processes prompt
- **Behavior**: Non-blocking, advisory only
- **Purpose**: Make Agent aware of relevant skills
...
Agent sees this output as additional context before processing the user's prompt.
...
Agent calls Edit/Write tool
    ↓
.agents/settings.json registers hook (matcher: Edit|Write)
    ↓
...
ELSE:
  Exit with code 0 (ALLOW)
    ↓
IF BLOCKED:
  stderr → Agent sees message
  Edit/Write tool does NOT execute
  Agent must use skill and retry
IF ALLOWED:
...
- **Exit code 2**: BLOCK (stderr → Agent)
- **Exit code 0**: ALLOW
...
Agent receives this message and understands it needs to use the skill before retrying the edit.
...
1. **Only way** to send message to Agent from PreToolUse
2. stderr content is "fed back to Agent automatically"
3. Agent sees the block message and understands what to do
...
Agent: "I'll create the user service..."
...
Prevent repeated nagging in the same session - once Agent uses a skill, don't block again.

### State File Location

`.agents/hooks/state/skills-used-{session_id}.json`
...
- If Agent doesn't use the skill but makes a different edit, it won't block again
- Trust that Agent follows the instruction
